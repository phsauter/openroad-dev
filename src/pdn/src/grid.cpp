                      ...
                      bool repair_rings)
{
  ...
  if (!repair_rings) {
    for (auto& [old_shape, new_shape] : replace_shapes) {
      old_shape->getGridComponent()->replaceShape(old_shape,
                                                  std::move(new_shape));
    }
    debugPrint(getLogger(),
               utl::PDN,
  ...
  auto overlaps_extension_edge
      = [](const Shape* shape, const odb::Rect& target, int side) {
          const odb::Rect& rect = shape->getRect();
          if (shape->isHorizontal()) {
            return side < 0 ? target.xMin() <= rect.xMin()
                            : target.xMax() >= rect.xMax();
          }
          if (shape->isVertical()) {
            return side < 0 ? target.yMin() <= rect.yMin()
                            : target.yMax() >= rect.yMax();
          }
          return false;
        };

  auto is_nearest_target = [&extension_side_and_distance,
                            &get_grid,
                            &overlaps_extension,
                            &overlaps_extension_edge](
                               const Shape* shape,
    ...
    for (const auto& other : target_shapes) {
      if (other == target || other->getType() != odb::dbWireShapeType::RING) {
        continue;
      }
      ...
      if (target_side != 0 && other_side == 0
          && overlaps_extension_edge(shape, other->getRect(), target_side)) {
        return false;
      }
...
std::vector<odb::Rect> Grid::getGridAreaRects() const
{
  return {getGridArea()};
}
  ...
  for (Connect* connect : connects) {
    ...
    for (const auto& lower_shape : lower_shapes) {
      ...
      for (auto it = upper_shapes.qbegin(
        ...
        if (repair_rings && lower_grid != nullptr && upper_grid != nullptr
            && lower_grid != upper_grid && has_pad_connect) {
          continue;
        }
        if (repair_rings && lower_grid != nullptr && upper_grid != nullptr
            && lower_grid->getDomain() != upper_grid->getDomain()) {
        ...
        if (repair_rings && type() != Grid::kExisting && lower_grid != this
            && upper_grid != this
            && (lower_shape->getType() == odb::dbWireShapeType::RING
                || upper_shape->getType() == odb::dbWireShapeType::RING)) {
          continue;
        }
        if (!lower_shape->getRect().overlaps(upper_shape->getRect())) {
...
bool Grid::makeVias(const Shape::ShapeTreeMap& global_shapes,
                    ...
                    bool repair_rings)
{
  makeVias(global_shapes, obstructions, extra_connects, repair_rings);

  // repair vias that are only partially overlapping straps
  const bool repaired = repairVias(
      global_shapes, local_obstructions, extra_connects, repair_rings);
  if (repaired) {
                    ...
                    bool repair_rings)
{
  ...
  getIntersections(vias, search_shapes, extra_connects, repair_rings);
  ...
  for (const auto& via : vias) {
    ...
    const bool ring_via
        = via->getLowerShape()->getType() == odb::dbWireShapeType::RING
          || via->getUpperShape()->getType() == odb::dbWireShapeType::RING;
    const bool ignore_same_net_obs
        = repair_rings && ring_via && lower_component != nullptr
          && upper_component != nullptr
          && lower_component->getGrid() != upper_component->getGrid();
    for (auto* layer : via->getConnect()->getIntermediteLayers()) {
      ...
      if (search_obs.qbegin(
              ...
              && bgi::satisfies([this,
                                 ...
                                 via_net,
                                 lower_component,
                                 upper_component](const ShapePtr& other) {
                   if (ignore_same_net_obs && other->getNet() == via_net) {
...
void Grid::getGridLevelObstructions(ShapeVectorMap& obstructions) const
{
  ...
  for (auto* layer : layers) {
    for (const auto& core : getDomainAreaRects()) {
...
InstanceGrid::InstanceGrid(
    ...
    const std::vector<odb::dbTechLayer*>& generate_obstructions)
    : Grid(domain, name, start_with_power, generate_obstructions), insts_(insts)
{
  for (auto* inst : insts_) {
    ...
    if (halo != nullptr && !halo->isSoft()) {
      ...
      // copy halo from db
      addHalo({std::max(halos_[0], halo_box.xMin()),
               std::max(halos_[1], halo_box.yMin()),
               std::max(halos_[2], halo_box.xMax()),
               std::max(halos_[3], halo_box.yMax())});
    }
  }
}
...
{
  if (insts_.size() == 1) {
    return getName() + " - " + getInstance()->getName();
  }
  return getName();
}

odb::PtrSet<odb::dbInst> InstanceGrid::getInstances() const
{
  return odb::PtrSet<odb::dbInst>(insts_.begin(), insts_.end());
}
...
odb::Rect InstanceGrid::getDomainArea() const
{
  ...
  inst_box.mergeInit();
  for (auto* inst : insts_) {
    inst_box.merge(inst->getBBox()->getBox());
  }
...
odb::Rect InstanceGrid::getDomainBoundary() const
{
  ...
  for (auto* inst : insts_) {
    odb::Rect inst_pin_box;
    inst_pin_box.mergeInit();

    for (auto* pin : inst->getMaster()->getMTerms()) {
      if (!pin->getSigType().isSupply()) {
        continue;
      }

      inst_pin_box.merge(pin->getBBox());
    }

    const odb::dbTransform transform = inst->getTransform();
    transform.apply(inst_pin_box);
    pin_box.merge(inst_pin_box);
  }
...
void InstanceGrid::getGridLevelObstructions(ShapeVectorMap& obstructions) const
{
  ...
  for (const auto& [layer, shapes] : local_obs) {
    for (const auto& inst_box : getGridAreaRects()) {
  ...
  for (auto* inst : insts_) {
    for (const auto& [layer, shapes] : getInstanceObstructions(inst, halos_)) {
      for (const auto& shape : shapes) {
        auto obs
            = std::make_shared<GridObsShape>(layer, shape->getRect(), this);
        local_obs[layer].push_back(obs);
      }
    }
  }
                                    ...
                                    bool repair_rings) const
{
  ...
  if (!getRings().empty()) {
    pin_owner = getRings().front().get();
  } else if (!getStraps().empty()) {
    pin_owner = getStraps().front().get();
  }
  ...
  Grid::getIntersections(vias, inst_shapes, extra_connects, repair_rings);
}
...
void InstanceGrid::checkHalo() const
{
  ...
  for (auto* inst : insts_) {
    ...
    // overlaps are skipped: no halo adjustment can clear those (the instance
    // is placed on top of them), so they are out of scope here.
    ...
    const Halo suggested = suggestHalo(inst_box, overlapping_rows);
  ...
  for (auto* inst : insts_) {
    for (auto* iterm : inst->getITerms()) {
      ...
      if (top != nullptr) {
        ...
        odb::PtrSet<odb::dbTechLayer> layers;
        for (auto* master_obs : inst->getMaster()->getObstructions()) {
